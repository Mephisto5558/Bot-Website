/* eslint-disable import-x/extensions, @typescript-eslint/no-unsafe-type-assertion */
...
export default class MongoStore extends MemoryStore {
  ...
  override async get<P extends Parameters<MemoryStore['get']>, CB extends P[1]>(
    ...
    const data = await this.db.get('website', `sessions.${sid}`) as (session & { passport?: Record<string, unknown> }) | undefined;
  ...
  override async set<P extends Parameters<MemoryStore['set']>, CB extends P[2]>(
    sid: P[0], sessionData: session & { passport?: { user?: Profile } }, callback?: CB
